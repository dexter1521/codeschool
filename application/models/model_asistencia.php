...
class model_asistencia extends CI_Model 
{
	...
	*/
	...
	{	
		...
		$insert_asistencia = array(
			'no_empleado' => $this->input->post('code'),
	...
	public function existStudent()
	{
		$code = $this->input->post('code'); // Obtiene el valor del campo 'code' del formulario
		$this->db->where('no_empleado', $code); // Agrega la condición WHERE a la consulta
		$query = $this->db->get('student'); // Ejecuta la consulta en la tabla 'student'
		$result = $query->row_array(); // Obtiene la primera fila del resultado como un array asociativo
		// Devuelve true si se encontró un registro, false en caso contrario
		return !empty($result);

	}

	public function checkSectionHour($sectionID){

		$actualDate= date('Y-m-d H:i:s');
		$actualHour = strtotime(date('H:i:s'));
		$this->db->select('fecha_inicio, fecha_fin, hora_inicio, hora_fin');
